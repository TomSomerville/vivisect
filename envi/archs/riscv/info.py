...
_cat_pat = r'(?:(' + r')|('.join(list(RISCV_CAT)) + r'))+'
_arch_pat = re.compile(r'^RV%s%s$' % (xlen_pat, cat_pat))
...
def _getRiscVArchInfo(description):
    ...
    """
    match = _arch_path.match(description)
    if match:
        # Return a single RISCV_CAT value that incorporates all of the category
        # flags
        cat = RISCV_CAT(RISCV_CAT[o] for o in match.groups[1:] if o is not None)
...
def getRiscVNumGPRs(description):
    ...
    """
    if RISCV_CAT.E in getRiscVCategories(description):
        return 16
    else:
        return 32
...
def getRiscVIALIGN(description):
    ...
    """
    if RISCV_CAT.C in getRiscVCategories(description):
        return 16
    else:
        return 32
...
def getRiscVFLEN(description):
    ...
    a description string.
    """
    if RISCV_CAT.F in getRiscVCategories(description):
        return 32
    elif RISCV_CAT.D in getRiscVCategories(description):
        return 64
    elif RISCV_CAT.Q in getRiscVCategories(description):
        return 128
    else:
        return None
